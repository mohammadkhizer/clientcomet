...
export default function EditServicePage() {
    ...
    try {
      ...
      if (data) {
        ...
        form.reset({
          ...
          iconName: data.iconName || (availableIcons.length > 0 ? availableIcons[0] : "Code"),
      ...
      toast({ title: "Error fetching service", description: (error as Error).message, variant: "destructive" });
    } finally {
      setIsFetching(false);
    }
  ...
  async function onSubmit(data: ServiceFormValues) {
    ...
    const updateData: Partial<CreateServiceData> = {
      title: data.title,
      description: data.description,
      longDescription: data.longDescription || "",
      iconName: data.iconName,
      status: data.status,
      features: data.features || [], // Zod transform handles conversion
    };

    try {
      const updatedService = await updateService(service.id, updateData);
      toast({
  ...
  if (!service && !isFetching) {
     return (
      <div className="text-center py-10">
        <p className="text-muted-foreground">Service not found or could not be loaded.</p>
  ...
  return (
    <FadeIn>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Update details for the service: {service?.title}</p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Modify information about the service.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <FormField control={form.control} name="features" render={({ field }) => (
                <FormItem><FormLabel>Key Features (comma-separated, Optional)</FormLabel><FormControl><Input placeholder="e.g., Feature 1, Feature 2, Feature 3" {...field} /></FormControl><FormMessage /></FormItem>
              )}/>
            ...
            <CardFooter className="flex justify-end">
              <Button type="submit" className="button-primary" disabled={isLoading || isFetching}>
                {isLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Updating...</>) : (<><Save className="mr-2 h-4 w-4" /> Update Service</>)}
              </Button>
            </CardFooter>
          </Card>
        </form>
